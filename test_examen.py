...
def test_get_min():
    ...
    assert lista == [{'1020': {'description': 'PSEG ALAMEDA 14 (DAVANT JARDÍ VIA CENTRAL) - VALÈNCIA', 'name': 'Albereda'}}, {'1021': {'description': 'PSEG ALAMEDA 1(DAVANT JARDÍ VIA CENTRAL) - VALÈNCIA', 'name': 'Pla del Reial'}}]

    lista=get_min("1000", diccionario)
    with pytest.raises(ValueError) as exc:
        exception(True)
    assert "No hay elementos" == str(exc.value)
